	...
	return (
		<header
			className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
				scrolled || menuOpen
					? "bg-virtus-black border-b border-virtus-gold/15 shadow-lg"
					: "bg-transparent"
			}`}
		>
			...
			>
				<div className="absolute inset-0 bg-virtus-black" />

				<div
					id="mobile-menu"
					className={`absolute inset-0 bg-virtus-black transition-transform duration-300 ${
				...
				>
					<div className="flex items-center justify-between px-4 py-4 border-b border-virtus-gold/15">
						...
						<button
							type="button"
							className="inline-flex items-center justify-center w-10 h-10 rounded-md text-virtus-gold hover:text-virtus-white transition-colors"
					...
					<div className="px-4 py-8 flex flex-col gap-3">
						{links.map((link) => (
							<a
								...
								href={link.href}
								onClick={closeMenu}
								className="text-virtus-gold text-lg font-sans-ui font-medium px-3 py-4 rounded-md border border-virtus-gold/15 hover:bg-virtus-gold hover:text-virtus-black transition-colors"
							>
								{link.label}
							</a>
						))}
